...
public class GUI implements ActionListener {
		...
		button.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				for (int i = 0; i < thePath.size(); i++) {
					...
					if (dungeon[obj.getX()][obj.getY()] != 3
							&& dungeon[obj.getX()][obj.getY()] != 4) {
						if (!(playerPositionX == obj.getY() && playerPositionY == obj
								.getX()))
							grid[obj.getX()][obj.getY()]
									.setBackground(findPath);
			...
			public void actionPerformed(ActionEvent e) {
				...
				for (int i = 0; i < dungeon.length; i++) {
					for (int j = 0; j < dungeon[i].length; j++) {
						if (dungeon[i][j] == 3 || dungeon[i][j] == 4) {// start
																		// and
																		// end
																		// points
	...
	public static void addGrid(GridBagConstraints c, final Container pane,
			int hei, int wid) {
		...
		for (int i = 0; i < height; i++) {
			for (int j = 0; j < width; j++) {
				...
				// 0 is a wall
				if (dungeon[i][j] == 0 && i > (playerPositionY - 2)
						&& i < (playerPositionY + 2) && j > playerPositionX - 2
						&& j < playerPositionX + 2) {
				...
				} else if (dungeon[i][j] == 1 && i > (playerPositionY - 2)
						&& i < (playerPositionY + 2) && j > playerPositionX - 2
						&& j < playerPositionX + 2) {
